...
#**************************** MBGD（开始） **************************** 
    ...
    def MSGD(self, training_data, epochs = 3000, mini_batch_size = 1, eta = 0.1, error = 0.01):
...
pd_file = pd.read_csv(file_data, sep =',')
...
MATRIX_INTERACTION = []
for ci in range(li):
    MATRIX_INTERACTION.append([])
    for co in range(lo):
        INTER_IO = 0
        INTER_IO += pd.DataFrame(BP.weights[0])[ci].sum()
