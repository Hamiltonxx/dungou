...
async def projects_export(request):
    ...
    wb = Workbook()
    sheet = wb.active
    sheet.append(("ID","工程名","施工单位","穿越地层","区间长度(m)","起止时间","盾构类型","盾构厂家/型号","土仓压力(MPa)","刀盘转速(r/min)","扭矩(kN.m)","推力(kN)","推进速度(mm/min)","土体改良","保压泵","双闸门","耐磨措施","平均进度(m/d)","刀具磨损","盾构直径(m)","盾构埋深(m)","稠度指数","开挖面水头(m)","渗透系数(cm/s)","等效石英含量(%)","限制粒径(mm)","最大粒径(mm)","省份","图片","刀盘直径(m)","刀盘形式","开口率(%)","中心鱼尾刀","先行刀","切削刀","刮刀","超挖刀","双刃滚刀","单刃滚刀","驱动方式","最大转速(r/min)","额定扭矩(kN.m)","脱困扭矩(kN.m)","泡沫","膨润土","最大推力(kN)","最大推速(mm/min)","螺旋机型式","直径(mm)","最大通过粒径(mm)","最大出土量(m3/h)","最大扭矩(kN.m)","螺旋机转速(r/min)"))
    conn = await aiomysql.connect(**mysqlconfig)
    cur = await conn.cursor()
